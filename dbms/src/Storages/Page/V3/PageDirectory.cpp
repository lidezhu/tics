...
template <typename Trait>
bool VersionedPageEntries<Trait>::updateRemotePage(const PageVersion & ver, const PageEntryV3 & entry)
{
    auto page_lock = acquireLock();
    if (type == EditRecordType::VAR_ENTRY)
    {
...
template <typename Trait>
typename PageDirectory<Trait>::PageEntries PageDirectory<Trait>::updateRemotePages(PageEntriesEdit && edit, const DB::PageStorageSnapshotPtr & snap_, const WriteLimiterPtr & write_limiter)
{
    std::unique_lock apply_lock(apply_mutex);
    auto seq = toConcreteSnapshot(snap_)->sequence;
    typename PageDirectory<Trait>::PageEntries ignored_entries;
    // TODO: write to wal
    UNUSED(write_limiter);
    {
        std::unique_lock table_lock(table_rw_mutex);
        ...
        for (const auto & r : edit.getRecords())
        {
            ...
            auto & version_list = iter->second;
            if (!version_list->updateRemotePage(PageVersion(seq, 0), r.entry))
            {

...
try
{
    ...
    });
    {
        ...
        edits.appendRecord({.type = PS::V3::EditRecordType::VAR_ENTRY, .page_id = "aaabbb", .entry = {.offset = 10}});
    ...
    {
        auto data_path = remote_dir + "/data_1";
        ASSERT_TRUE(Poco::File(data_path).exists());
        Poco::File(data_path).remove();
        ASSERT_FALSE(Poco::File(data_path).exists());
        auto page = page_storage->read("aaabbb");
        ASSERT_TRUE(page.isValid());
        ASSERT_EQ("nahida opened her eyes", String(page.data.begin(), page.data.size()));
    }
...
try
{
    ...
    {
        auto edits = PS::V3::universal::PageEntriesEdit{};
        edits.appendRecord({.type = PS::V3::EditRecordType::VAR_ENTRY, .page_id = page_id1, .entry = {.offset = 5}});
        edits.appendRecord({.type = PS::V3::EditRecordType::VAR_ENTRY, .page_id = page_id2, .entry = {.offset = 10}});
        writer->writeEditsAndApplyRemoteInfo(edits);
    }
    writer->writeSuffix();
    writer.reset();

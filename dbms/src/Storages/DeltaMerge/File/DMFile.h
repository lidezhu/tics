...
public:
    ...
    String path() const { return !file_path.empty() ? file_path : parent_path + (status == Status::READABLE ? "/dmf_" : "/.tmp.dmf_") + DB::toString(file_id); }
    ...
    const String         encryptionBasePath() const { return !file_path.empty() ? file_path : parent_path + "/dmf_" + DB::toString(file_id); }

...
{
    PageIdU64 next_segment_id = 1; // DELTA_MERGE_FIRST_SEGMENT_ID
    SegmentMetaInfos segment_infos;
    while (next_segment_id != 0)
    {
        Segment::SegmentMetaInfo segment_info;
        auto target_id = UniversalPageIdFormat::toFullPageId(UniversalPageIdFormat::toFullPrefix(StorageType::Meta, ns_id), next_segment_id);
        auto page = temp_ps->read(target_id);
        ReadBufferFromMemory buf(page.data.begin(), page.data.size());
        readSegmentMetaInfo(buf, segment_info);
        next_segment_id = segment_info.next_segment_id;
    ...
    WriteBatches & wbs)
{
    ...
    {
        LOG_DEBUG(parent_log, "Restore segment begin. Delta id {} stable id {} range {} epoch {} next_segment_id {}", segment_info.delta_id, segment_info.stable_id, segment_info.range.toDebugString(), segment_info.epoch, segment_info.next_segment_id);
        auto stable = StableValueSpace::createFromCheckpoint(context, temp_ps, remote_store_id, ns_id, segment_info.stable_id, wbs);
        auto delta = DeltaValueSpace::createFromCheckpoint(context, temp_ps, segment_info.range, remote_store_id, ns_id, segment_info.delta_id, wbs);

        auto new_segment_id = context.storage_pool->newMetaPageId();
        auto segment = std::make_shared<Segment>(parent_log, segment_info.epoch, segment_info.range.shrink(range), new_segment_id, /* next_segment_id */ 0, delta, stable);
        segments.push_back(segment);
        LOG_DEBUG(parent_log, "Restore segment end. Delta id {} stable id {} range {} epoch {} next_segment_id {}", segment_info.delta_id, segment_info.stable_id, segment_info.range.toDebugString(), segment_info.epoch, segment_info.next_segment_id);
    }
    return segments;
}

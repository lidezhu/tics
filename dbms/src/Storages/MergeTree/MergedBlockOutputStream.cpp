    ...
    for (size_t i = 0; i < columns_list.size(); ++i, ++it)
    {
        ...
        if (permutation)
        {
            ...
            if (primary_columns_name_to_position.end() != primary_column_it)
            {
                ...
                writeData(column.name, *it->type, primary_column, offset_columns, false, serialization_states[i]);
            }
            else
            {
                /// We rearrange the columns that are not included in the primary key here; Then the result is released - to save RAM.
                ColumnPtr permutted_column = column.column->permute(*permutation, 0);
                writeData(column.name, *it->type, *permutted_column, offset_columns, false, serialization_states[i]);
            }
        }
        else
        {
            writeData(column.name, *it->type, *column.column, offset_columns, false, serialization_states[i]);
        }
    }

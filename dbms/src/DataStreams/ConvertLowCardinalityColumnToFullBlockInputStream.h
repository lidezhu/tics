...
/** Combines several sources into one.
  ...
  */
class ConvertLowCardinalityColumnToFullBlockInputStream : public IProfilingBlockInputStream
{
public:
    explicit ConvertLowCardinalityColumnToFullBlockInputStream(const BlockInputStreamPtr & input)
    {
...
private:
    Block convert(Block && block) const
    {
        ...
        {
            auto * type_with_dict = typeid_cast<const DataTypeLowCardinality *>(column.type.get());
            auto * col_with_dict = typeid_cast<const ColumnLowCardinality *>(column.column.get());

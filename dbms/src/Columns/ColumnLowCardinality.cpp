...
ColumnLowCardinality::ColumnLowCardinality(MutableColumnPtr && column_unique_, MutableColumnPtr && indexes_)
...
StringRef ColumnLowCardinality::serializeValueIntoArena(size_t n, Arena & arena, char const *& begin, std::shared_ptr<TiDB::ITiDBCollator> collator, String & sort_key_container) const
{
    return getDictionary().serializeValueIntoArena(getIndexes().getUInt(n), arena, begin, collator, sort_key_container);
}

const char * ColumnLowCardinality::deserializeAndInsertFromArena(const char * pos, std::shared_ptr<TiDB::ITiDBCollator>)
{
    compactIfSharedDictionary();
...
ColumnLowCardinality::Dictionary::Dictionary(MutableColumnPtr && column_unique_)
    : column_unique(std::move(column_unique_))
{
    checkColumn(*column_unique);
}
ColumnLowCardinality::Dictionary::Dictionary(ColumnPtr column_unique_)
    : column_unique(std::move(column_unique_))
{
    checkColumn(*column_unique);
}

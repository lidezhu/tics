...
public:
    ...
    {
        return column_holder->serializeValueIntoArena(n, arena, begin, collator, sort_key_container);
    }
    ...
    {
        return getNestedColumn()->updateHashWithValue(n, hash, collator, sort_key_container);
    }
    ...
    {
        return column_holder->byteSize(offset, limit);
    }
    ...
    , index(numSpecialValues(is_nullable), 0)
{
    if (column_holder->size() < numSpecialValues())
        throw Exception("Too small holder column for ColumnUnique.", ErrorCodes::LOGICAL_ERROR);
    if (column_holder->isColumnNullable())
        throw Exception("Holder column for ColumnUnique can't be nullable.", ErrorCodes::LOGICAL_ERROR);
    ...
    size_t max_dictionary_size)
{
    ...
    if (src_column == nullptr)
        throw Exception("Invalid column type for ColumnUnique::insertRangeFrom. Expected " + column_holder->getName() +
                        ", got " + src.getName(), ErrorCodes::LOGICAL_ERROR);
...
UInt128 ColumnUnique<ColumnType>::IncrementalHash::getHash(const ColumnType & column)
{
    ...
    if (column_size != num_added_rows.load())
    {
        ...
        for (size_t i = 0; i < column_size; ++i)
            column.updateHashWithValue(i, sip_hash, nullptr, TiDB::dummy_sort_key_contaner);

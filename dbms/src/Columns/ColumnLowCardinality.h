...
class ColumnLowCardinality final : public COWPtrHelper<IColumn, ColumnLowCardinality>
{
    ...
    ColumnLowCardinality(MutableColumnPtr && column_unique, MutableColumnPtr && indexes);
...
public:
    ...
    {
        return ColumnLowCardinality::create(column_unique_->assumeMutable(), indexes_->assumeMutable());
    }
    ...
    {
        return getDictionary().updateHashWithValue(getIndexes().getUInt(n), hash, collator, sort_key_container);
    }
    ...
    {
        return idx.getPositions()->byteSize(offset, limit) + getDictionary().byteSize();
    }

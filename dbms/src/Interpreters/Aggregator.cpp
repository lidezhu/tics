    ...
    AggregateDataPtr overflow_row) const
{
    ...
    if constexpr (Method::low_cardinality_optimization)
        state.init(key_columns, aggregation_state_cache, collators);
    else
...
//        if constexpr (!Method::low_cardinality_optimization)
        key = state.getKey(key_columns, params.keys_size, i, key_sizes, keys, *aggregates_pool, sort_key_containers);
    ...
    AggregateDataPtr overflow_row) const
{
    ...
    typename Method::State state;
    if constexpr (Method::low_cardinality_optimization)
        state.init(key_columns, aggregation_state_cache, params.collators);
    else

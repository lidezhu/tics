...
#include <DataStreams/ConvertLowCardinalityColumnToFullBlockInputStream.h>
...
void InterpreterSelectQuery::executeAggregation(Pipeline & pipeline, const ExpressionActionsPtr & expression, const FileProviderPtr & file_provider, bool overflow_row, bool final)
{
    pipeline.transform([&](auto & stream)
    {
        stream = std::make_shared<ConvertLowCardinalityColumnToFullBlockInputStream>(std::make_shared<ExpressionBlockInputStream>(stream, expression));
    });
